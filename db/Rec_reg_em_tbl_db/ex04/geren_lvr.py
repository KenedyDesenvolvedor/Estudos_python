...
def inserir_dados(conexao):
    ...
    livros = [Livro('Python para Iniciantes', 'John Doe', 39.99),
              ...
              Livro('Inteligencia Artificial', 'Alan Turine', 59.99)]
    
    clientes = [Cliente('Alice', 'maravilhas@pais.com'),
                Cliente('Bob', 'quadrado@fenda.bikini'),
                Cliente('Son Goku','dragon@boll.com')]
    
    pedidos = [Pedido(1, 1, 2, '2023-06-15'),
               Pedido(2, 2, 2, '2023-06-16'),
    ...
    for pedido in pedidos:
        cursor.execute('''INSERT INTO Pedidos (cliente_id,
                       livro_id,
                       quantidade,
                       data_pedido)
